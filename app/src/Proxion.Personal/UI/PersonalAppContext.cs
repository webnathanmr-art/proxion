...
/// <summary>
/// Owns the whole personal-fork session: the settings window, the tray icon, and (once
/// PURPLE is launched) the same process-tree-scoped proxy rule as the general app.
/// PURPLE can be launched either by clicking the window's button, or automatically on
/// startup if the "auto-run" setting was previously enabled - either way, the window
/// hides to the tray once launched, and the tray icon can always bring it back.
/// </summary>
public sealed class PersonalAppContext : ApplicationContext
{
    ...
    public PersonalAppContext()
    {
        ...
        _statusMenuItem = menu.Items.Add("Not started");
        ...
        _trayIcon = new NotifyIcon
        {
            ...
            Text = TrimTrayText("Proxion - not started"),
    ...
    private void OnLaunchRequested(object? sender, string purplePath)
    {
        ...
        UpdateStatus($"Routing via {ProxyLabel()}");
    ...
    private void UpdateStatus(string text)
    {
        ...
        _trayIcon.Text = TrimTrayText($"Proxion - {text}");
